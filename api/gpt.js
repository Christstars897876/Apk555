...
exports.initialize = async ({ req, res, font }) => {
    ...
    // Protection anti-bug : Si le texte est trop long, on le coupe proprement
    if (query && query.length > 3000) {
        query = query.substring(0, 3000) + "... (texte tronqué)";
    }
    ...
    // Garde uniquement les 6 derniers échanges (12 messages) pour ne pas saturer l'API externe
    ...
    // Construction du contenu du message (Support texte + image)
    let userContent = [];
    ...
    // Ajout de la photo si elle est présente (Transmission par URL pour éviter l'erreur de surcharge)
    if (imageUrl) {
        userContent.push({
            "type": "image_url",
    ...
    // Configuration de l'identité (Minato Namikaze / Style Gemini Flash) et des règles d'images
    const systemPrompt = "You are Minato Namikaze (the Fourth Hokage). Act and respond with the lightning-fast intelligence, efficiency, and clarity of Gemini 1.5 Flash / Gemini 2.0 Flash. Always detect the language of the user's message and reply fluently in that exact same language (e.g. if French, reply in French). Crucial Rule: If the user asks you to draw, create, generate, or imagine an image, you MUST end your response by appending exactly: [IMAGINE: detailed prompt in English]. Example: 'Voici votre image! [IMAGINE: a cute orange cat wearing ninja headband, anime style, 4k]'. If no image is requested, reply normally WITHOUT the tag.";

    // Assemblage final du corps de la requête avec l'historique (Mémoire)
    const body = {
        "messages": [
    ...
    const getResponse = async () => {
        try {
            ...
            // Affiche l'origine exacte du blocage dans votre console de serveur
            console.error('⚠️ Erreur détaillée API GPT:', error.response ? error.response.status : error.message);
    ...
    try {
        ...
        if (response && response.success && response.message) {
            ...
            // Sauvegarde dans l'historique pour la mémoire (On ne sauvegarde que le texte pour rester léger)
            if (query) {
                conversationHistory[userId].push({ "role": "user", "content": query });
                conversationHistory[userId].push({ "role": "assistant", "content": answer });
            }

            // Formatage des textes en gras si disponible
            if (font && font.bold) {
            ...
            throw new Error('Réponse invalide reçue de l\'API externe');
        }
    } catch (error) {
        // Fallback sécurisé en cas de coupure réseau ou token expiré
        return res.json({
